...
def vggvox_resnet2d_icassp(input_dim=(257, 250, 1), num_class=8631, mode='train', net_args=None):
    net=net_args.get('net')
    loss=net_args.get('loss')
    vlad_clusters=net_args.get('vlad_cluster')
    ghost_clusters=net_args.get('ghost_cluster')
    bottleneck_dim=net_args.get('bottleneck_dim')
    aggregation = net_args.get('aggregation_mode')
